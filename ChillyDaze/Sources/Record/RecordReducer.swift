...
@Reducer public struct RecordReducer {
    // MARK: - State
    public struct State: Equatable {
        ...
        public init() { self.chills = .initialized }
    }
    ...
    public var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case let .getChillsResult(.success(chills)):
                state.chills = .loaded(
                    chills.sorted(by: { e0, e1 in
                        if let t0 = e0.photo?.timestamp {
                            if let t1 = e1.photo?.timestamp { return t0 > t1 }
                            return true
                        }

                        if let _ = e1.photo?.timestamp { return false }
