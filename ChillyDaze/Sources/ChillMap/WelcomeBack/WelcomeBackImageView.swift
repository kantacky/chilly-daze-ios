...
struct WelcomeBackImageView: View {
    ...
    init(image: Image?, chillRate: CGFloat) {
        self.image = image
        self.chillRate = chillRate
    }
    init(uiImage: UIImage?, chillRate: CGFloat) {
        ...
        Group {
            if let image = image {
                ZStack {
                    ...
                    VStack(spacing: 0) {
                        Spacer()
                        ChillyIndicator(chillRate: chillRate).frame(width: 252)
...
#Preview { WelcomeBackImageView(image: nil, chillRate: 0.67) }
